...
router.post('/', requireAuth, async (req, res) => {
  const { name, icon, description, price, category_id, sort_order } = req.body || {};
  if (!name) return res.status(400).json({ error: 'Name is required' });
  if (!category_id) return res.status(400).json({ error: 'Category is required' });
  const { rows } = await pool.query(
    'INSERT INTO products (name, icon, description, price, category_id, sort_order) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *',
    [name, icon || '🌿', description || '', price || 0, category_id, sort_order || 0]
  );
...
router.put('/:id', requireAuth, async (req, res) => {
  ...
  const { rows } = await pool.query(
    ...
    [name, icon, description, price, category_id, sort_order, req.params.id]
  );
